    ...
    <div class="card-body">
      <form method="post" action="{{route('category.update',$category->id)}}">
        ...
        <div class="form-group">
          ...
          <input id="inputTitle" type="text" name="title" placeholder="Ingrese un nombre"  value="{{$category->title}}" class="form-control">
          @error('title')
      ...
      <div class="form-group {{(($category->is_parent==1) ? 'd-none' : '')}}" id='parent_cat_div'>
        ...
        <div class="form-group">
          ...
          <select name="status" class="form-control">
              ...
              <option value="inactive" {{(($category->status=='inactive')? 'selected' : '')}}>Deshabilitado</option>
          </select>
          @error('status')
          <span class="text-danger">{{$message}}</span>
          @enderror
